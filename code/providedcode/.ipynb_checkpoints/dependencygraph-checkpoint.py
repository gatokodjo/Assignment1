...
class DependencyGraph(object):
    ...
    Raises:
        ...
        DependencyGraphError: _description_
        ValueError: _description_
    ...
    def from_sentence(sent):
        """
        Create a DependencyGraph from a plain text sentence by
        performing tokenization and POS tagging using NLTK.
    ...
    def __init__(
        self, tree_str=None, cell_extractor=None, zero_based=False, cell_separator=None
    ):
        """Dependency graph.
    ...
    def remove_by_address(self, address):
        """
        Removes a node from the dependency graph by its address.
        This method deletes the node at the specified address from the nodes
        dictionary. Note that this operation does not clean up references to
        the removed node that may exist in other nodes, potentially leaving
        dangling references.
        Args:
            address (int): The unique address/identifier of the node to remove.
        Raises:
            KeyError: If no node exists at the specified address.
        Warning:
            After removal, other nodes may still contain references to the
            removed node's address, which could lead to inconsistent graph state.
            Consider using a method that also cleans up references if available.
        """
        del self.nodes[address]

    def redirect_arcs(self, originals, redirect):
        """
        Redirects dependency arcs from multiple original nodes to a single target node.
        
        For each node in the dependency graph, this method updates the dependency list
        by replacing any references to nodes in the 'originals' list with a reference
        to the 'redirect' node. This is commonly used in dependency parsing operations
        such as arc-standard transitions where multiple nodes need to be consolidated.
        
        Args:
            originals (list): List of node addresses whose incoming arcs should be redirected.
                             These are the source nodes that will have their dependencies
                             transferred to the redirect node.
            redirect (int/str): The target node address where all arcs pointing to any
                               node in 'originals' will be redirected to.
        
        Returns:
            None: This method modifies the dependency graph in-place.
        
        Example:
            If node A depends on nodes [1, 2, 3] and we call redirect_arcs([2], 5),
            then node A will now depend on nodes [1, 5, 3].
        """
    ...
    def _parse(
        self, input_, cell_extractor=None, zero_based=False, cell_separator=None
    ):
        """Parse a sentence.
        ...
        if not self.nodes[0]["deps"]["ROOT"]:
            raise DependencyGraphError(
                "The graph does'n contain a node that depends on the root element."
            )
    ...
    def _tree(self, i):
        ...
        word = node["word"]
        deps = list(chain.from_iterable(list(node["deps"].values())))
    ...
    def tree(self):
        ...
        word = node["word"]
        deps = chain.from_iterable(list(node["deps"].values()))
        return Tree(word, [self._tree(dep) for dep in deps])
    ...
    def nx_graph(self):
        ...
        nx_edgelist = [(n, self._hd(n)) for n in nx_nodelist if self._hd(n)]
        nx_labels = {}
